...
const CreateNotification = () => {
  ...
  const handleChangeDepartment = (event) => {
    ...
    // Get the account IDs of the filtered users
    const updateFilteredUser = filteredUser.map((item) => item.accountId);
  
    // Combine the account IDs from all currently selected departments
    const combinedSelectedUsers = users
      .filter((user) => updatedDepartmentId.includes(user.departmentId))
      .map((item) => item.accountId);
  
    if (combinedSelectedUsers.length === 0) {
      // Reset selected users to an empty array when no departments are checked
      setSelectedUsers([]);
    } else {
      // Filter the selected users based on the currently selected departments
      const updatedSelectedUsers = selectedUsers.filter((userId) =>
        combinedSelectedUsers.includes(userId)
      );
      setSelectedUsers(updatedSelectedUsers);
    }
  
    setUpdateFilteredUsers((prevUpdateFilteredUsers) => [
      ...new Set([...prevUpdateFilteredUsers, ...filteredUser])
    ]);
  };
