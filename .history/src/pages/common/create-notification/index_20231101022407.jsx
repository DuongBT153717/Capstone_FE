...
const CreateNotification = () => {
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedFiles.length > 0) {
      const acceptedFileTypes = ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/pdf', 'text/csv'];
      const invalidFiles = [];
      ...
      if (invalidFiles.length === 0) {
        ...
        toast.error(`Invalid file types: ${invalidFiles.join(', ')}. Please select Word, PDF, or CSV files.`);
      }
    }
  };
                      ...
                      <Grid item xs={7}>
                        <Typography mb={2}>Attach file: </Typography>
                        {
                          file.length > 0 && file.map((item, index) => (
                            <Typography mb={2} ml={2} key={index}>{item.name}</Typography>
                          ))
                        }
                        <input
                          id="customFileInput"
